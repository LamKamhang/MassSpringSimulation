...
int main(int argc, char const *argv[])
{
	...
	// ChainSystem1D test(mass, springs);
		...
		shared_ptr<Particle> m1(new Particle(10, true));
	shared_ptr<Particle> m2(new Particle(10, false, {1, 0, 0}));
	...
	// shared_ptr<Spring> s2(new Spring(4, 2, m2, m3));
	...
	{
		test.update();
		unsigned x0 = m1->getPos().getItem(0)*5;
		unsigned x1 = m2->getPos().getItem(0)*5;
		cout << m1->getPos().getItem(0) << " " << m2->getPos().getItem(0) << endl;
		// render(x0, x1);
		usleep(100000);
	}
